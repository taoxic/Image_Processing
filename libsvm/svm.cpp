...
//分配内存，主要是为处理多类着想。
	...
	//点乘两个样本数据， 按 svm_node 中 index (一般为特征)进行运算， 一般来说，
	//index 中 1， 2， …直到-1。返回点乘总和。
...
protected:
	// 计算时实际参加运算的样本数目，经过 shrink 处理后，该数目会小于全部样本总数
	int active_size;
	//样本所属类别，该值只取+1/-1 。虽然可以处理多类，最终是用两类 SVM 完成的。
	schar *y;
	//梯度
	...
	int l;
	bool unshrink;	// XXX
	//返回对应于样本的 C。 设置不同的 Cp 和 Cn 是为了处理数据的不平衡。
	double get_C(int i)
	{
		return (y[i] > 0)? Cp : Cn;
	}
